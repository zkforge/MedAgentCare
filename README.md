...
flowchart LR
    ...
    Router --> Lead[LeadAgent 复杂度判断与任务拆分]
    Lead --> Single[单 Agent 路径]
    Lead --> Swarm[Swarm 多 Agent 协作]
    Single --> Consultation[ConsultationAgent]
    Swarm --> Consultation
    Swarm --> Diagnostic[DiagnosticAgent]
    Swarm --> Research[ResearchAgent]
    Consultation --> Loop[AgentLoop + SkillRegistry]
    Diagnostic --> Loop
    Research --> Loop
    Loop --> Skills[9 个本地 Skill]
    Skills --> Knowledge[Milvus Lite 知识库]
    Skills --> Memory[短期记忆 / Mem0]
    Loop --> Guard[医疗安全约束与自动修复]
    Swarm --> Synthesis[LeadAgent 结果汇总]
    Guard --> Output[结构化回答]
    Synthesis --> Output
```
